...
from NeuralPainter.add_pages.forms import PainterForm

from datetime import datetime
from datetime import timedelta


@pytest.mark.parametrize('death_date, birth_date, valid',
                         [(datetime.now(), datetime.now() + timedelta(days=100), False),
                         (datetime.now() + timedelta(days=100), datetime.now(), True)])
@pytest.mark.django_db
@override_settings(MEDIA_ROOT=tempfile.gettempdir())
def test_painter_form_clean(death_date, birth_date, valid):
    ...
    form = PainterForm(data=data, files={'photo': image})
